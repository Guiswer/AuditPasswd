...
def _derive_key(master_password, salt):
    """Internal function for Key Stretching via PBKDF2."""
    kdf = PBKDF2HMAC(
        algorithm=hashes.SHA256(),
        length=32,
        salt=salt,
        iterations=600000, # OWASP/NIST recommendation
    )
    return base64.urlsafe_b64encode(kdf.derive(master_password.encode()))

def encrypt_data(data, master_password):
    """Authenticated Encryption: Salt (16B) + Ciphertext."""
    salt = os.urandom(16)
...
def decrypt_data(combined_data, master_password):
    """Decryption with integrity verification (HMAC)."""
    try:
